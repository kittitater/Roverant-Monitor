...
export default function MyRoversPage() {
  ...
  function handleModalChange(e) {
    ...
    if (fieldName === "ip_address") {
      ...
      errorKey = `edit${fieldName.charAt(0).toUpperCase() + fieldName.slice(1)}`;
    }
    setErrors({ ...errors, [errorKey]: "" });
  }
  ...
  async function handleSave() {
    if (!editFormData.is_owner) {
      setErrorMsg("Only the owner can update the rover.");
      return;
    }
  ...
  async function handleDelete() {
    if (!editFormData.is_owner) {
      setErrorMsg("Only the owner can delete the rover.");
      return;
    }
  ...
  async function handleShare() {
    ...
    try {
      const idToken = await user.getIdToken();
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/rover/${editFormData.rover_id}/share`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!res.ok) {
        const errData = await res.json();
        setShareError(errData.detail || "Failed to share rover.");
        return;
      }
      const data = await res.json();
  ...
  const renderName = (value, rowData) => (
    <div
      onClick={() => openModal(rowData)}
      className="text-black text-base cursor-pointer py-2 pr-5 w-full h-full"
    >
      {value} {rowData.is_owner ? "" : "(Shared)"}
    </div>
  );
    ...
    <div className="mx-auto min-h-svh min-w-full space-y-8">
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        ...
        <DialogContent className="p-8 sm:rounded-2xl max-w-2xl">
          ...
          <div className="grid grid-cols-3 space-x-5 mt-5">
            <div className="col-span-2">
              ...
              <input
                ...
                onChange={handleModalChange}
                placeholder="Roverant Rover 99"
                disabled={!editFormData.is_owner}
                className={`mt-2 block w-full rounded-xl bg-white px-3.5 py-2 text-base text-gray-900 focus:outline-none focus:ring-2 focus:ring-black ${
                  errors.editName
                    ? "ring-2 ring-red-500 focus:ring-red-500"
                    : "ring-2 ring-gray-300 focus:ring-black"
                } placeholder:text-gray-400 ${!editFormData.is_owner ? "opacity-50 cursor-not-allowed" : ""}`}
              />
              {errors.editName && (
                <p className="mt-1 text-sm text-red-600">{errors.editName}</p>
              )}
          ...
          {editFormData.is_owner && (
            <div className="mb-5">
              <label className="block text-sm font-semibold text-gray-900">
                Share Rover with User (Email)
              </label>
              <div className="flex space-x-5 mt-2">
                <input
                  type="email"
                  ...
                  className="block rounded-xl bg-white px-3.5 py-2 text-base text-gray-900 ring-2 ring-gray-300 focus:ring-black focus:outline-none"
                />
                <button
                  type="button"
                  onClick={handleShare}
                  className="block w-32 rounded-xl bg-black px-3.5 py-2.5 text-sm font-semibold text-white hover:bg-white hover:text-black hover:ring-2 hover:ring-black"
                >
                  Share
                </button>
                <button
          ...
          <div className="flex items-center justify-between space-x-4 mt-3">
            <Button
              ...
              disabled={!editFormData.is_owner}
              className={`rounded-xl font-semibold text-white hover:bg-white hover:text-red-500 hover:ring-2 hover:ring-red-500 ${
                !editFormData.is_owner ? "opacity-50 cursor-not-allowed" : ""
              }`}
              variant="destructive"
            >
              <svg
